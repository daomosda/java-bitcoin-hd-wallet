...
public class ChainIndexRepository {
    ...
    private static String className = "ChainIndexRepository";
    ...
    private static final ReentrantLock GLOBAL_DB_WRITE_LOCK = new ReentrantLock(true);
        ...
        String sql =
            ...
            try (
                PreparedStatement ps = conn.prepareStatement(sql); 
                    ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) {
                    AppLogger.warn(className, " getStoredTip: " +
                ...
                AppLogger.info(className, " getStoredTip → {}",
                        tip.label());
                return tip;
            }
    }
    ...
    public List<String> getTxidsAboveHeight(Connection conn,
                                              int forkHeight)
            throws SQLException {
        ...
        AppLogger.info(className, "getTxidsAboveHeight({}) → {} txids",
    ...
    public void deleteUtxosCreatedAboveHeight(Connection conn,
                                                int forkHeight)
            throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(
            ...
            AppLogger.info(className, "[ReorgDetector] deleteUtxosCreatedAboveHeight({}): " +
    ...
    public StoredBlock getBlockAtHeight(Connection conn, int height)
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) {
                    AppLogger.info(className, "No block at height={}", height);
                    return null;
                }
    ...
    public void standAlonerollbackToHeight(int forkHeight) throws SQLException, Exception {
        ...
        try {
            ...
            AppLogger.info(className, " Orphaned txids above height {}: {}",
            ...
            if (newTipHash != null) {
                ...
                AppLogger.warn(className, " No block found at forkHeight={} — tip NOT updated",
                    forkHeight);
            }

            sqLteDb.commit(conn);      //conn.commit();
            AppLogger.warn(className, " Rolled back to height={} ({} orphan txids removed)",
                forkHeight, orphanTxids.size());
            ...
            AppLogger.error(className, " standAlonerollbackToHeight({}) FAILED: {}", forkHeight, e.getMessage());
            throw e;        
        } finally {
    ...
    public void rollbackToHeight(Connection conn, int forkHeight) throws SQLException, Exception {
            ...
            if (newTipHash != null) {
                ...
                AppLogger.info(className, " No block found at forkHeight={} — tip NOT updated",
                    forkHeight);
            }

            AppLogger.info(className, " Rolled back to height={} ({} orphan txids removed)",
                forkHeight, orphanTxids.size());
    }
    ...
    public void deleteBlocksAboveHeight(Connection conn, int forkHeight)
        ...
        try (PreparedStatement ps = conn.prepareStatement(
            ...
            ps.setInt(1, forkHeight);
            int rows = ps.executeUpdate();
            AppLogger.info(className, " deleteBlocksAboveHeight({}): deleted {} block(s) " +
    ...
    public Map<String, Object> getTransactionDetail(String txid)
        ...
        try {
            ...
            if (tx == null) {
                AppLogger.warn(className, " getTransactionDetail: txid={} not found", txid);
                return null;
            }
...
//txid, vout, value_sat, script_hex, script_type, address
    ...
    public int updateUtxosBatch(List<Utxo> utxos) throws SQLException {
        ...
        try {
            ...
            try {
                conn.rollback();
                AppLogger.error(className, "Rolled back batch due to error", e);
            } catch (SQLException rollbackEx) {
                AppLogger.error(className, "Failed to rollback transaction", rollbackEx);
            }
            ...
            try {
                ...
                AppLogger.warn(className, "Failed to restore auto-commit setting", e.getMessage());
            }
        }
    ...
    */
    ...
    public void markAsSpent(String spendingTxid, List<SpentInput> spentInputs) throws SQLException {
        ...
        try {
            ...
            try {
                ...
                AppLogger.error(className, "Failed to rollback", rollbackEx);
            }
            ...
            try {
                ...
                AppLogger.warn(className, "Failed to restore auto-commit", e.getMessage());
            }
        }
    }
     ...
     */
    public void unspendUtxosSpentByTxids(Connection conn,
                                           List<String> orphanedTxids)
            throws SQLException {
        if (orphanedTxids.isEmpty()) {
            AppLogger.info(className, "No orphaned txids — nothing to unspend");
            return;
        }
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            int rows = ps.executeUpdate();
            AppLogger.info(className, "unspendUtxosSpentByTxids: restored {} row(s)", rows);
        }
    }
     ...
     */
    public void deleteTransactionsAboveHeight(Connection conn,
                                                int forkHeight)
            throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(
                "DELETE FROM transactions WHERE block_height > ?")) {
            ps.setInt(1, forkHeight);
            int rows = ps.executeUpdate();
            AppLogger.info(className, "deleteTransactionsAboveHeight({}): " +
                      "deleted {} row(s)", forkHeight, rows);
        }
    }
    
    public void updateChainTip(Connection conn, String hash, int height)
            throws SQLException {
        try (PreparedStatement ps = conn.prepareStatement(
            ...
            ps.setInt   (2, height);
            ps.executeUpdate();
            AppLogger.info(className, "updateChainTip → height={} hash={}",
