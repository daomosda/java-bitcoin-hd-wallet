...
import com.bitcoin.hdwallet.trezorhardware.Bip32Deriv;
import com.bitcoin.hdwallet.trezorhardware.BipPathParser;
import java.io.*;
import java.util.*;
...
public final class Psbt {
    ...
    public static final class PsbtInput {
        ...
        private byte[] finalScriptSig;
        private byte[] finalScriptWitness;
        ...
        /**
    * Add to PsbtInput — reads back the raw bip32Derivs map populated by
    * addBip32Deriv(), parsing each value back into fingerprint + path.
    *
    * Mirrors PsbtUtils.buildBip32DerivValue()'s encoding:
    *   bytes[0..3]   = master fingerprint
    *   bytes[4..]    = path components, each a 4-byte little-endian uint32
         * @return 
    */
    public List<Bip32Deriv> readBip32Derivs() {
        List<Bip32Deriv> result = new ArrayList<>();

        for (Map.Entry<ByteArrayWrapper, byte[]> entry : bip32Derivs.entrySet()) {
            byte[] pubkeyBytes = entry.getKey().getData();     //getBytes();
            byte[] value       = entry.getValue();

            if (value.length < 4 || (value.length - 4) % 4 != 0) {
                throw new IllegalStateException(
                        "Malformed bip32Deriv value: length=" + value.length);
            }

            String fingerprintHex = HexUtils.bytesToHex(
                    Arrays.copyOfRange(value, 0, 4));

            int numComponents = (value.length - 4) / 4;
            int[] pathInts = new int[numComponents];
            for (int i = 0; i < numComponents; i++) {
                int offset = 4 + (i * 4);
                pathInts[i] = (value[offset] & 0xFF)
                            | ((value[offset + 1] & 0xFF) << 8)
                            | ((value[offset + 2] & 0xFF) << 16)
                            | ((value[offset + 3] & 0xFF) << 24);
            }

            result.add(new Bip32Deriv(
                    HexUtils.bytesToHex(pubkeyBytes),
                    fingerprintHex,
                    BipPathParser.format(pathInts)));
        }
       ...
       public void addPartialSig(
        ...
        public Map<byte[], byte[]> getPartialSignatures() {
            ...
            for (Map.Entry<ByteKey, byte[]> e : entries.entrySet()) {
                ...
                if (type == PSBT_IN_PARTIAL_SIG) {
                    ...
                    byte[] pubkey = Arrays.copyOfRange(keyBytes, 1, keyBytes.length);
                    byte[] sig    = e.getValue();
        ...
        public void merge(PsbtInput other) {
            partialSigs.putAll(other.partialSigs);
    ...
    public void merge(Psbt signed) {
        for (int i = 0; i < signed.inputs.size(); i++) {

            // Safety check: Ensure your local list is long enough
            if (i < inputs.size()) {

                // Get items using the index 'i'
                PsbtInput mine = inputs.get(i);
                PsbtInput theirs = signed.inputs.get(i);
