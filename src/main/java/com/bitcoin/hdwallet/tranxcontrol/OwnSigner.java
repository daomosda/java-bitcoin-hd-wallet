...
import com.bitcoin.hdwallet.bitcoinhwi.HwiSigner;
...
import com.bitcoin.hdwallet.trezorhardware.HardwareSigner;
...
public class OwnSigner {
    ...
    private static String className = "OwnSigner";
    ...
    private final TrezorSigner trezorSigner;
    ...
    public OwnSigner(BitcoinRpcClient walletRpc, BitcoinRpcClient nodeRpc, 
            TrezorSigner trezorSigner, FundingGuard fundingGuard, CoinSelector coinSelector, 
    ...
    public PsbtCreateResult createFundedPsbt(
        ...
        AppLogger.info(className, " destAddress={}", destinationAddress);
        AppLogger.info(className, " changeAddress={}", changeAddress);
        AppLogger.info(className, " amount={}", amountBtc);
        ...
        AppLogger.info(className, "Created PSBT: fee={} BTC changepos={}", fee.toString() + ","  + Integer.toString(changePos));
    ...
    public void verifyUtxoSolvable(String txid, int vout) throws Exception {
        ...
        for (int i = 0; i < utxos.length(); i++) {
            ...
            if (u.getString("txid").equals(txid) && u.getInt("vout") == vout) {
                ...
                AppLogger.info(className, "[verify] txid={}:{} solvable={} spendable={} desc={}",
    ...
    public String sendCoins(String destAddress, BigDecimal amountBtc, 
            String changeAddress, int targetBlocks, List<Utxo> selectedUtxos) throws Exception {
    ...
    */
    ...
    public String sendHwiCoins(
        String        destAddress,
        BigDecimal    amountBtc,
        String        changeAddress,
        int targetBlocks, List<Utxo> selectedUtxos) throws Exception {
        
        // ✅ Verify change address is not a mining address
        AppLogger.info(className, "[sendHwiCoins()] destAddress={}",   destAddress);
        AppLogger.info(className, "[sendHwiCoins()] changeAddress={}",  changeAddress);
        AppLogger.info(className, "[sendHwiCoins()] amountBtc={}",      amountBtc);

        if (changeAddress == null || changeAddress.isBlank()) {
            throw new IllegalStateException(
                    "[sendHwiCoins()] changeAddress is null or blank."
        ...
        AppLogger.info(className, "[sendHwiCoins()] changepos={} changeAddress={}",
                changePos, changeAddress);
        
        AppLogger.info(className, "[sendHwiCoins()] PSBT created. fee={} changepos={}",
            createRes.getBigDecimal("fee"),
            createRes.optInt("changepos", -1));

        String unsignedPsbt = createRes.getString("psbt");
        
        String hwiDevicesJson = runHwiEnumerate();
        System.out.println("HWI enumerate result: " + hwiDevicesJson);

        if ("[]".equals(hwiDevicesJson)) {
            AppLogger.info(className, "No hardware wallet detected");
            AppLogger.info(className, "Falls back to software signing");
            ...
            // 3) Sign with your own signer and update PSBT
            String signedPsbtBase64 = signPsbtWithOwnSigner(unsignedPsbt, decoded);
            //System.out.println("[signedPsbtBase64] " + signedPsbtBase64);
        ...
        // "fingerprint=98667b10"
        String masterFingerprint = addressManager.getMasterFingerPrint();
        HwiSigner hwiSigner = new HwiSigner(masterFingerprint);   //"98667b10");   

        // 3. Sign with Hardware Wallet
        String signedPsbtBase64 = hwiSigner.signPsbt(unsignedPsbt);
        ...
        AppLogger.info(className, " PSBT outputs:");
        for (int i = 0; i < txVout.length(); i++) {
            ...
            AppLogger.info(className, "  vout={} addr={} amount={} isOurChange={}",
                    i, outAddr, outAmt, isChange);
        }

        if (changePos < 0) {
            AppLogger.warn(className, " No change output — full amount consumed by fee?");
        }

        JSONObject fin = (JSONObject) BitcoinRpcClient.executeRpc(
                "finalizepsbt", signedPsbtBase64, true);

        if (!fin.getBoolean("complete")) {
            throw new IllegalStateException("[sendHwiCoins()] PSBT not complete");
        }
    ...
    public String sendTrezorCoins(
        ...
        // ✅ Verify change address is not a mining address
        AppLogger.info(className, "[sendTrezorCoins()] destAddress={}",   destAddress);
        AppLogger.info(className, "[sendTrezorCoins()] changeAddress={}",  changeAddress);
        AppLogger.info(className, "[sendTrezorCoins()] amountBtc={}",      amountBtc);

        if (changeAddress == null || changeAddress.isBlank()) {
            throw new IllegalStateException(
                    "[sendTrezorCoins()] changeAddress is null or blank."
                    + " Call addressManager.getNextChangeAddress() first.");
        }        
        ...
        // ✅ Verify Core used our change address
        int changePos = createRes.optInt("changepos", -1);
        AppLogger.info(className, "[sendTrezorCoins()] changepos={} changeAddress={}",
                changePos, changeAddress);
        
        AppLogger.info(className, "[sendTrezorCoins()] PSBT created. fee={} changepos={}",
            createRes.getBigDecimal("fee"),
            createRes.optInt("changepos", -1));

        String unsignedPsbt = createRes.getString("psbt"); 
        
        List<HardwareDeviceInfo> devices = (List<HardwareDeviceInfo>) CachedWalletMnemMap.getObject("devices");
        if (devices.isEmpty()) {
            AppLogger.info(className, "No hardware wallet detected");
            AppLogger.info(className, "Falls back to software signing");
        ...
        String signedPsbtBase64 = signPsbtWithHardwareOrFallback(unsignedPsbt, decoded, trezorSigner);
    ...
    public String signPsbtWithOwnSigner(String unsignedPsbtBase64, JSONObject decoded) throws Exception {
        ...
        System.out.println("[signPsbtWithOwnSigner()] pubkeyHex=" + pubkeyHex);
        System.out.println("[signPsbtWithOwnSigner()] input=0 path=" + path + " masterFp=" + masterFpHex);
   ...
   public void ensureInputsHaveSolvingData(
       ...
       if (selectedUtxos == null || selectedUtxos.isEmpty()) {
           AppLogger.warn(className, "ensureInputsHaveSolvingData:"
                   + " no UTXOs provided.");
           return;
       }

       AppLogger.info(className, "ensureInputsHaveSolvingData: Checking solving data for {} input(s)...",
               selectedUtxos.size());
       ...
       AppLogger.info(className, "ensureInputsHaveSolvingData: fingerprint={} coinType={}",
       ...
       for (Utxo utxo : selectedUtxos) {
           ...
           AppLogger.info(className, "ensureInputsHaveSolvingData:] addr={} solvable={} hasHdPath={} desc={}",
                   address, solvable, hasHdPath,
                   desc.length() > 50 ? desc.substring(0, 50) + "..." : desc);

           if (solvable && hasHdPath) {
               AppLogger.info(className, "ensureInputsHaveSolvingData: ✅ {} already has solving data.",
                       address);
               continue;
           }

           // ── Needs reimport — get derivation path ──────────────────────────
           AppLogger.warn("ensureInputsHaveSolvingData: {} missing solving data — preparing"
                   + " reimport...", address);
           ...
           if (path == null) {
               AppLogger.error("ensureInputsHaveSolvingData: Cannot determine derivation path"
                       + " for {} — skipping. This input may fail to sign.",
                       address);
               continue;
           }

           AppLogger.info(className, "ensureInputsHaveSolvingData: Resolved path for {}: {}",
                   address, path);
           ...
           if (changeAndIndex == null) {
               AppLogger.error("ensureInputsHaveSolvingData: Could not parse change/index"
                       + " from path: {}", path);
               continue;
           }
           ...
           AppLogger.info(className, "ensureInputsHaveSolvingData: change={} index={}", changeComponent, index);
           ...
           AppLogger.info(className, "ensureInputsHaveSolvingData: Reimporting as: {}...",
       ...
       if (reimportBatch.length() == 0) {
           AppLogger.info(className, "ensureInputsHaveSolvingData: All inputs already have solving data ✅");
           return;
       }

       AppLogger.info(className, "ensureInputsHaveSolvingData: Reimporting {} input(s) with solving data...",
       ...
       for (int i = 0; i < results.length(); i++) {
           ...
           if (r.getBoolean("success")) {
               ...
               failed++;
               AppLogger.error( className,"ensureInputsHaveSolvingData: Reimport failed at index {}: {}",
                       i, r.optJSONArray("error"));
           }
       }

       AppLogger.info(className, "ensureInputsHaveSolvingData: Reimport complete: {} success {} failed",
               success, failed);
       ...
       for (Utxo utxo : selectedUtxos) {
           ...
           AppLogger.info(className, "ensureInputsHaveSolvingData: Final check addr={} solvable={} desc={}",
                   utxo.getAddress(), solvable,
                   newDesc.length() > 50 ? newDesc.substring(0, 50) + "..." : newDesc);

           if (!solvable) {
               throw new IllegalStateException(
                       "ensureInputsHaveSolvingData: Input still not solvable after reimport: "
                       + utxo.getAddress()
                       + " — check fingerprint and derivation path.");
           }
       }

       AppLogger.info(className, "ensureInputsHaveSolvingData: ✅ All inputs have solving data.");
   }
    ...
    private int[] parseChangeAndIndex(String path) {
        try {
            ...
            if (parts.length < 6) {
                AppLogger.warn("Path too short to parse: {}", path);
                return null;
            }
            ...
            AppLogger.error("Cannot parse change/index from path {}: {}",
                    path, e.getMessage());
            return null;
        }
    }
    ...
    private String extractPathFromDesc(String desc) {
        ...
        try {
            ...
            AppLogger.warn("extractPathFromDesc failed: {}",
                    e.getMessage());
            return null;
        }
    }
    ...
    private String findPathByPubkeyMatch(
        ...
        AppLogger.info(className, "Scanning HD paths to find match for {}...",
                address);
            ...
            for (int idx = 0; idx < SCAN_LIMIT; idx++) {
                try {
                    ...
                    if (address.equals(derived)) {
                        AppLogger.info(className, "Found match: {} at path {}",
                                address, path);
                        return path;
                    }

                } catch (Exception e) {
                    AppLogger.error(className, "Path scan error at change={}"
                            + " idx={}: {}", changeComp, idx, e.getMessage());
                }
            }
        }

        AppLogger.warn("No path found for {} within scan limit",
                address);
        return null;
    }
    ...
    private String deriveAddress(byte[] compressedPubKey, int coinType) {
        try {
            ...
            throw new RuntimeException("deriveAddress failed: "
                    + e.getMessage(), e);
        }
    }        
    ...
    private List<Utxo> collectChangeUtxos() throws Exception {
        ...
        AppLogger.info(className, " Checking {} used change address(es)...",
        ...
        for (int i = 0; i < utxos.length(); i++) {
            ...
            if (coinbase && confs < 100) {
                AppLogger.info(className, " Skipping immature coinbase:"
                        + " txid={}:{} confs={}",
            ...
            if (!spendable || !safe) {
                AppLogger.warn(className, " Skipping non-spendable: {}:{}",
                        u.getString("txid").substring(0, 12) + "...",
                        u.getInt("vout"));
                continue;
            }
            ...
            if (amount.compareTo(MIN_CONSOLIDATION_VALUE) < 0) {
                AppLogger.warn(className, " Skipping dust UTXO: {} BTC"
    ...
    private void ensureAllHaveSolvingData(List<Utxo> utxos)
        ...
        for (Utxo utxo : utxos) {
            ...
            AppLogger.info(className, " Reimporting {} as wpkh(xpub/index)...",
                    utxo.getAddress());
            ...
            if (keyRecord == null) {
                throw new IllegalStateException(
                        " No key record for change address: "
                        + utxo.getAddress());
            }
        ...
        if (reimportBatch.length() > 0) {
            ...
            for (int i = 0; i < results.length(); i++) {
                JSONObject r = results.getJSONObject(i);
                if (!r.getBoolean("success")) {
                    AppLogger.warn(className, " Reimport failed at {}: {}",
                            i, r.optJSONArray("error"));
                }
            }
            AppLogger.info(className, " Reimported {} address(es) with"
    ...
    private void printConsolidationReport(List<Utxo> utxos) {
        AppLogger.info(className, " ── Change UTXO Report ─────────────");
        BigDecimal total = BigDecimal.ZERO;
        for (Utxo u : utxos) {
            total = total.add(u.getAmount());
            AppLogger.info(className, "  addr={} txid={}:{}"
                    + " amount={} BTC confs={}",
                    u.getAddress(),
        ...
        AppLogger.info(className, "  ─────────────────────────────────");
        AppLogger.info(className, "  Total: {} BTC across {} UTXO(s)",
                total, utxos.size());
        AppLogger.info(className, " ─────────────────────────────────────");
    }
   ...
   public String convertChangeToSpendableUtxo() throws Exception {
       ...
       AppLogger.info(className, " Found {} change UTXO(s) totalling {} BTC",
       ...
       AppLogger.info(className, " Consolidation target address: {}",
        ...
        AppLogger.info(className, "[consolidate] Estimated vbytes={} minFee={} BTC"
                + " totalInput={} BTC",
        ...
        */
        ...
        AppLogger.info(className, 
                " Estimated vbytes={} minFee={} BTC (2x buffer)"
                + " totalInput={} BTC",
                estimatedVBytes, minFeeBtc, totalInput);

        if (totalInput.compareTo(minFeeBtc) <= 0) {
            throw new IllegalStateException(String.format(
                    " Total input (%s BTC) too small to cover"
        ...
        AppLogger.info(className, " Creating consolidation PSBT...");
        ...
        AppLogger.info(className, " PSBT created."
        ...
        for (int i = 0; i < psbtVout.length(); i++) {
            ...
            String     outAddr = out.getJSONObject("scriptPubKey")
                                    .optString("address", "");
            BigDecimal outAmt  = out.getBigDecimal("value");
            AppLogger.info(className, "  vout={} addr={} amount={} BTC",
                    i, outAddr, outAmt);
        }
       ...
       AppLogger.info(className, " Signing {} input(s)...",
               psbtInputs.length());
       ...
       if (!fin.getBoolean("complete")) {
           ...
           for (int i = 0; i < diagInputs.length(); i++) {
               ...
               AppLogger.info(className, " Input {} signed={} addr={}",
                       i, hasSig,
                       i < changeUtxos.size()
                               ? changeUtxos.get(i).getAddress() : "?");
           }
           throw new IllegalStateException(
                   " PSBT finalization incomplete."
                   + " Check signing keys.");
       }
       ...
       AppLogger.info(className, " ✅ Consolidation broadcasted!"
               + " txid={}", txid);
       AppLogger.info(className, " {} change UTXOs → 1 spendable UTXO",
               changeUtxos.size());
       ...
       //    AppLogger.info(className, "[consolidate] Mined 1 block to confirm.");
       //}
   ...
   private String signAllInputs(
        ...
        AppLogger.info(className, " Signing with fingerprint={}",
                ourFingerprint);

        // ── Skip walletprocesspsbt — watch-only wallet cannot sign ────────────
        // Go straight to manual HD signing
        AppLogger.info(className, " Using manual HD signing"
        ...
        for (int i = 0; i < decodedIns.length(); i++) {
            ...
            if (alreadySigned) {
                AppLogger.warn(className, " Input {} already signed.", i);
                continue;
            }

            if (bip32Derivs == null || bip32Derivs.length() == 0) {
                AppLogger.warn(className, " Input {} has no bip32_derivs"
                        + " — cannot sign.", i);
                continue;
            }
            ...
            AppLogger.info(className, " Input {} path={} masterFp={}",
                    i, path, masterFp);

            if (!ourFingerprint.equalsIgnoreCase(masterFp)) {
                AppLogger.info(className, " Input {} fingerprint mismatch:"
                        + " ours={} psbt={} — skipping.",
            ...
            if (!psbtInput.has("witness_utxo")) {
                AppLogger.warn(className, " Input {} missing witness_utxo.",
                        i);
                continue;
            }
            ...
            AppLogger.info(className, " Input {} signed. sig={}...",
        ...
        AppLogger.info(className, " Manual signing complete.");
        return psbt.toBase64();
    }
   ...
   private static long btcToSats(BigDecimal btc) {
    ...
    /**
     * Runs "hwi enumerate" and returns the raw JSON output as a String.
     * Returns "[]" if the command fails or no devices are found.
     * @return 
     */
    public static String runHwiEnumerate() {
        // If hwi is not on PATH, replace "hwi" with full path, e.g.:
        // String hwiCmd = "C:\\Users\\CONALDES\\AppData\\Roaming\\Python\\Python314\\Scripts\\hwi.exe";
        String hwiCmd = "hwi";

        ProcessBuilder pb = new ProcessBuilder(hwiCmd, "enumerate");
        ...
        try {
            ...
            if (exitCode != 0) {
                System.err.println("[HwiClient] hwi enumerate exited with code " + exitCode
                        + ". Output: " + output);
                return "[]";
            }
            ...
            System.err.println("[HwiClient] Error running hwi enumerate: " + e.getMessage());
            return "[]";
        }
    }
    
    /**
    * Drop-in alternative to signPsbtWithOwnSigner() — tries hardware
    * signer first, falls back to OwnSigner's manual HD signing.
    *
    * Add this method to your existing OwnSigner class.
        * @param unsignedPsbtBase64
        * @param decoded
        * @param hardwareSigner
        * @return 
        * @throws java.lang.Exception 
    */
   public String signPsbtWithHardwareOrFallback(
           String         unsignedPsbtBase64,
           JSONObject     decoded,
           HardwareSigner hardwareSigner) throws Exception {

       if (hardwareSigner != null) {
           AppLogger.info(className, " Attempting hardware wallet signing..."); // fall through to software signing below
           if (!hardwareSigner.isConnected()) {
               hardwareSigner.connect();
           }
           String signed = hardwareSigner.signPsbt(unsignedPsbtBase64);
           AppLogger.info(className, " ✅ Signed via "
                   + hardwareSigner.getDeviceName());
           return signed;
       }

       // ── Fallback: existing watch-only manual signing ───────────────────
       AppLogger.info(className, " Using software fallback (OwnSigner manual HD signing).");
       return signPsbtWithOwnSigner(unsignedPsbtBase64, decoded);
   }
}
