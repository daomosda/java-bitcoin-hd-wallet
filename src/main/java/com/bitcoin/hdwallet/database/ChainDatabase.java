...
public class ChainDatabase {
    ...
    private static String className = "ChainDatabase";
    ...
    public ChainDatabase(String dbPath) {
        ...
        AppLogger.info(className, " Initialized with path: {}", dbPath);
    }
    ...
    public Connection getConnection() throws SQLException {
        ...
        if (conn == null || conn.isClosed()) {
            ...
            try (Statement stmt = conn.createStatement()) {
                // ✅ Wait up to 10 seconds if DB is locked — instead of failing immediately
            ...
            AppLogger.info(className, " New connection created for thread = {}",
                    threadName);
        }
    ...
    // In Database.java or SQLiteConnectionManager.java
    // When creating each connection add:
    ...
    public void releaseThreadConnection() {
        ...
        if (conn != null) {
            ...
            try { conn.close(); } catch (SQLException e) {
                AppLogger.warn(className, " Error closing thread connection: {}",
                    e.getMessage());
            }
            
            String threadName = Thread.currentThread().getName();
            threadLocalConnection.remove();
            AppLogger.info(className, " Connection released for thread = {}",
                threadName);
        }
    }
    ...
    public void beginTransaction(Connection conn) throws SQLException {
        if (!conn.getAutoCommit()) {
            ...
            AppLogger.warn(className, " beginTransaction: already in transaction, skipping");
            return;
        }
    ...
    public void rollback(Connection conn) {
        try {
            ...
            AppLogger.error(className, " Rollback failed: {}", e.getMessage());
        }
    }
    ...
    public void runMigrations() throws SQLException {
        ...
        try {
            ...
            commit(conn);
            AppLogger.info(className, " Migrations complete");
        } catch (SQLException e) {
            rollback(conn);
            throw e;
        }
    ...
    public void shutdown() {
        ...
        AppLogger.info(className, " All connections closed");
    }
