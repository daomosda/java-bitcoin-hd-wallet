...
public class P2PServer {
    ...
    private static String className = "P2PServer";
    ...
    public void start() throws IOException {
        ...
        AppLogger.info(className, " Listening on port {}", port);
    }
    ...
    public void stop() {
        ...
        AppLogger.info(className, " Stopped");
    }
    ...
    private void acceptLoop() {
        while (running) {
            try {
                ...
                if (running) {
                    AppLogger.error(" Accept error: {}", e.getMessage());
                }
            }
        }
    }
        ...
        try (BufferedReader reader = new BufferedReader(
            ...
            while ((line = reader.readLine()) != null && running) {
                ...
                try {
                    ...
                    AppLogger.error(" Failed to handle message from {}: {}",
                        peerId, e.getMessage());
                }
            }

        } catch (IOException e) {
            AppLogger.error(" Connection closed: {} ({})",
                peerId, e.getMessage());
        } finally {
    ...
    private void sendToSocket(Socket socket, Message message) {
        try {
            ...
            writer.println(json);
            AppLogger.info(className, " → {} : type={}", socket.getRemoteSocketAddress(),
                message.getType());
        } catch (IOException e) {
            AppLogger.error(" Send failed: {}", e.getMessage());
        }
    }
}