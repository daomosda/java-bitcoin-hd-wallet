...
public class NetworkManagerImpl implements NetworkManager {
    ...
    private static String className = "NetworkManagerImpl";
    // ── Peer registry ─────────────────────────────────────────────────
    ...
    // timeouts
    private static final long PEER_WAIT_MS    = 10_000L;   //60_000L;
    ...
    public void addPeer(Peer peer) {
        ...
        AppLogger.info(className, " Peer connected: id={} pubKey={}",
            peer.getId(),
            peer.getPubKey() != null
    ...
    public void removePeer(String peerId) {
        ...
        AppLogger.info(className, " Peer disconnected: id={}", peerId);
    }
    ...
    public void onMessageReceived(Message message) {
        AppLogger.info(className, " Message received: type={} from={}",
        ...
        switch (message.getType()) {
            ...
            case LN_PUBKEY -> {
                ...
                AppLogger.info(className, " LN pubkey received from {}: {}",
            ...
            case CHANNEL_ID -> {
                ...
                AppLogger.info(className, " Channel ID received: {}",
                    message.getPayload());
            }

            default ->
                AppLogger.warn(className, " Unhandled message type: {}",
                    message.getType());
        }
    }
    ...
    public String waitForLnPubKey() throws InterruptedException {
        AppLogger.info(className, " Waiting will timeout after {} seconds for LN pubkey.", (PEER_WAIT_MS/1_000L));
        
        String pubKey = lnPubKeyQueue.poll(PEER_WAIT_MS, TimeUnit.MILLISECONDS);
        if (pubKey == null) {
            AppLogger.info(className, " Ensure a peer node is running on another computer, then select this option again.");
            throw new InterruptedException(
                "Timeout waiting for LN pubkey after " + PEER_WAIT_MS + " ms");
        }

        AppLogger.info(className, " Customer LN pubkey received: {}",
    ...
    public String getPeerByPubKey(String pubKeyStr) {
        ...
        if (peerId == null) {
            AppLogger.warn(className, " getPeerByPubKey: no peer found for {}",
    ...
    public String findMerchantPeer() throws InterruptedException {
        AppLogger.info(className, " Looking for merchant peer...");
        ...
        for (Peer peer : connectedPeers.values()) {
            if (peer.isMerchant()) {
                AppLogger.info(className, " Merchant peer found: {}", peer.getId());
                return peer.getId();
            }
        }
        ...
        AppLogger.info(className, " Using first peer as merchant: {}", peerId);
        return peerId;
    }
    ...
    public void sendToPeer(String peerId, String payload) {
        ...
        if (peer == null) {
            AppLogger.warn(className, " sendToPeer: peer not found id={}", peerId);
            return;
        }
        Message msg = new Message(Message.Type.LN_PUBKEY, payload, localId);
        peer.sendMessage(msg);
        AppLogger.info(className, " Sent LN_PUBKEY to peer={}", peerId);
    }
    ...
    public InvoicePayload waitForInvoice() throws InterruptedException {
        AppLogger.info(className, " Waiting for invoice from merchant... [waiting time {} second]", (MESSAGE_WAIT_MS/1_000L));
        ...
        AppLogger.info(className, " Invoice received: hash={} amount={} sat",
    ...
    public String waitForChannelId() throws InterruptedException {
        AppLogger.info(className, " Waiting for channel ID from merchant... [waiting time {} second]", (MESSAGE_WAIT_MS/1_000L));
        ...
        AppLogger.info(className, " Channel ID received: {}", channelId);
        return channelId;
    }
    ...
    private InvoicePayload parseInvoicePayload(String json) {
        ...
        try {
            ...
            AppLogger.error(className, " Failed to parse invoice payload: {}",
                e.getMessage());
            return null;
        }
    }
