...
public class ReorgAwareDatabase {
    ...
    private static String className = "ReorgAwareDatabase";
    ...
    public void rollbackToHeight(int forkHeight) throws SQLException {
        ...
        try {
            ...
            AppLogger.info(className, "Orphaned txids above height {}: {}", forkHeight, orphanTxids.size());
            ...
            if (newTip != null) {
                ...
                AppLogger.info(className, "New tip set: height={} hash={}",
            ...
            AppLogger.error(className, "Rollback to height {} FAILED: {}", forkHeight, e.getMessage());
            throw e;
        } finally {
    ...
    public void unspendUtxosSpentBy(Connection conn, List<String> orphanedTxids)
        ...
        if (orphanedTxids.isEmpty()) {
            AppLogger.warn(className, "No orphaned txids — nothing to unspend");
            return;
        }
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            AppLogger.info(className, "Unspent {} UTXO(s) consumed by orphaned txs", rows);
        }
    }
    ...
    public void deleteUtxosCreatedAbove(Connection conn, int forkHeight)
        ...
        int rows = sqLteDb.executeUpdate(conn,
            ...
            forkHeight);
        AppLogger.info(className, "Deleted {} UTXO(s) created above height {}", rows, forkHeight);
    }
    ...
    public void deleteBlocksAboveHeight(Connection conn, int forkHeight)
        ...
        int rows = sqLteDb.executeUpdate(conn,
            ...
            forkHeight);
        AppLogger.info(className, "Deleted {} block(s) above height {} (cascade applied)",
            rows, forkHeight);
    }
    ...
    public StoredBlock getBlockAtHeight(Connection conn, int height)
        ...
        if (row == null) {
            AppLogger.info(className, "No block found at height={}", height);
            return null;
        }
    ...
    public void setChainTip(Connection conn, String hash, int height)
        ...
        AppLogger.info(className, "chain_tip → height={} hash={}", height, hash);
    }
    ...
    public void rollback(int forkHeight) throws SQLException {
        ...
        try {
            ...
            conn.commit();
            AppLogger.info(className, "Rollback complete: height={} ({} orphaned txids removed)",
                forkHeight, orphanedTxids.size());

        } catch (SQLException e) {
            conn.rollback();
            AppLogger.error(className, "Rollback FAILED at height {}: {}", forkHeight, e.getMessage());
            throw e;
        } 
    }
