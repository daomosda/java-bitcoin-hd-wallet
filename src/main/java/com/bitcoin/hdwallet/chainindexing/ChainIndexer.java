...
public class ChainIndexer implements Runnable {
    ...
    private static String className = "ChainIndexer";
    ...
    public ChainIndexer(BitcoinRpcClient rpc, ChainDatabase sqLteDb, 
        ...
        this.reorgDetector = new ReorgDetector(this.rpc, this.sqLteDb, this.store);
        ...
        try {
            ...
            while (true) {
                try {
                    ...
                    // 2. Perform the task
                    AppLogger.info(className, " Executing task...");
                    incrementalSync();
                    //Thread.sleep(1500); // Simulate work
                    AppLogger.info(className, " Task finished.");
    ...
    public void initialSync() throws Exception {
        ...
        try {
            ...
            AppLogger.info(className, "Local tip: {}  Core tip: {}", localHeight, coreHeight);

            if (localHeight >= coreHeight) {
                AppLogger.info(className, "Already at tip — nothing to sync");
                return;
            }

            AppLogger.info(className, "Syncing {} blocks ({} → {})",
            ...
            AppLogger.error(className, "[Sync] Error: {}", e.getMessage());
            throw e;        
        } finally {
    ...
    private void incrementalSync() throws Exception {
        ...
        if (reorg.reorgOccurred() && !reorg.success()) {
            AppLogger.error(className, "Reorg recovery failed — pausing sync");
            return;
        }
        ...
        try {
            ...
            if (localHeight < coreHeight) {
                AppLogger.info(className, "Applying {} new block(s) ({} → {})",
                    coreHeight - localHeight, localHeight + 1, coreHeight);
                applyRange(conn, localHeight + 1, coreHeight);
            }
            sqLteDb.commit(conn);
    ...
    private void applyRange(Connection conn, int from, int to) throws Exception {
        for (int h = from; h <= to; h++) {
            ...
            if (total % BATCH_LOG_INTERVAL == 0) {
                AppLogger.info(className, " {} blocks applied (height {})", total, h);
            }
        }
    }
    ...
    private void applyBlock(Connection conn, BlockData block) throws Exception {
            ...
            //conn.commit();
            AppLogger.info(className, " Block {} committed ({} txs)",
        ...
        for (TxInputData vin : tx.inputs()) {
            ...
            try (PreparedStatement ps = conn.prepareStatement(
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (!rs.next()) {
                        AppLogger.warn("Prev output {}:{} not found — fee=0",
