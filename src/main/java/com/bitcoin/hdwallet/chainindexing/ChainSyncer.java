...
public class ChainSyncer implements Runnable {
    ...
    private static String className = "ChainSyncer";
    ...
    public ChainSyncer(BitcoinRpcClient rpc,
        ...
        this.reorgDetector = new ReorgDetector(rpc, sqLteDb, store);
    }
    ...
    public void stop() {
        AppLogger.info(className, "Stopping Chain sync scheduler...");
        scheduler.shutdown();
    ...
    public void tick() throws Exception {
        ...
        if (reorg.reorgOccurred() && !reorg.success()) {
            AppLogger.error(className, " Reorg failed — skipping tick");
            return;
        }
    ...
    public void catchUp() throws Exception {
        ...
        try {
            ...
            if (localHeight >= coreHeight) {
                AppLogger.info(className, " Already at tip height={}", localHeight);
                return;
            }
            ...
            AppLogger.info(className, " Syncing {} block(s) [{} → {}]",
    ...
    private void validateChainLink(Connection conn, int height, BlockData block) throws Exception {
