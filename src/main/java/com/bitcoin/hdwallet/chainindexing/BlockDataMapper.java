...
public final class BlockDataMapper {
    ...
    private static String className = "BlockDataMapper";
    ...
    public static BlockData mapBlock(JSONObject raw) {
        ...
        if (txArr == null) {
            AppLogger.warn(" Block {} has no 'tx' array",
                raw.optString("hash", "?"));
        } else {
            for (int i = 0; i < txArr.length(); i++) {
                ...
                if (elem instanceof JSONObject txObj) {
                    ...
                    AppLogger.warn(className, " tx[{}] is a String — " +
    ...
    private static long getFee(List<TxInputData> inputs,
        ...
        long inputSum = inputs.stream()
            .mapToLong(in -> {
                ...
                if (prev.isEmpty()) {
                    AppLogger.warn(className, "[getFee] Missing prevValue for input: {} (vout={})."
        ...
        if (fee < 0) {
            AppLogger.warn(className, "[getFee] Negative fee detected: inputs={} sats, outputs={} sats, fee={} sats."
    ...
    private static OptionalLong prevoutValueSat(TxInputData in) {
        ...
        try {
            ...
            AppLogger.info(className, "[prevoutValue] No prevout data for {}:{} (both gettxout and getrawtransaction returned null)",
                    in.prevTxId(), in.prevVout());
        } catch (BitcoinRpcException | JSONException e) {
            AppLogger.error(className, "[prevoutValue] Failed to fetch prevout for {}:{} - {}",
    ...
    public static BigDecimal parseBtcValue(JSONObject vout) {
        ...
        try {
            ...
            AppLogger.warn(" Unparseable value '{}' — defaulting 0", raw);
