...
public class TransactionBroadcaster {
    ...
    private static String className = "TransactionBroadcaster";
        ...
        for (int attempt = 0; attempt < maxRetries; attempt++) {
            ...
            try {
                ...
                if (!isRetryableBroadcastError(e)) {
                    AppLogger.error(className, "Non-retryable broadcast error: {}", e.getMessage());
    ...
    private void updateDatabaseAfterBroadcast(String rawTxHex, List<SpentInput> spentInputs) {
        try {
            ...
            AppLogger.error(className, "Failed to update database after broadcast", e);
