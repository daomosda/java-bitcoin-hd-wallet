...
public class BitcoinRpcClient {
    ...
    private static String className = "BitcoinRpcClient";
    ...
    public BitcoinRpcClient(String host, int port, String rpcUser, String rpcPassword, String walletName) {
        ...
        AppLogger.info(className, "BCJSONRpchttp: " + this.endpoint.toString());
    ...
    */
    ...
    public void ensureWalletExistsAndLoaded(String walletName) throws BitcoinRpcException, Exception {
        ...
        if (isWalletLoaded(walletName)) {
            AppLogger.info(className, "[Wallet] '" + walletName + "' already loaded.");
            return;
        }
    ...
    public void createWatchOnlyDescriptorWallet(String walletName) throws BitcoinRpcException {
        AppLogger.info(className, "[Wallet] Creating watch-only descriptor wallet '{}' ...", walletName);
    ...
    private void createWallet(String name) throws Exception {
        AppLogger.info(className, "[Wallet] Creating wallet '" + name + "' ...");
        Object result = executeRpc("createwallet",
                           walletName,
        ...
        AppLogger.info(className, "[Wallet] Created wallet '" + name + "'");
    }
    */
    ...
    */
    ...
    private void loadWallet(String name) throws BitcoinRpcException {
        AppLogger.info(className, "[Wallet] Loading wallet '" + name + "' ...");
    ...
    */
    ...
    private void rollbackToCommonAncestor(Connection conn, ChainIndexRepository chainStore, int localHeight) throws Exception {
        ...
        AppLogger.warn(className, "Reorg: no common ancestor found, full reindex required");
    } 
    /**
    ...
    private void handleNetworkException(NetworkException e, String method) {
        ...
        switch (e.getReason()) {
            ...
            case CONNECTION_REFUSED -> {
                AppLogger.error(className, "✗ Bitcoind is not running or wrong port.");
                AppLogger.error(className, "  Host    : " + e.getHost());
                AppLogger.error(className, "  Port    : " + e.getPort());
                AppLogger.error(className, "  Method  : " + method);
                AppLogger.error(className, "  Action  : Start bitcoind and retry.");
            }
            case HOST_UNREACHABLE -> {
                AppLogger.error(className, "✗ Host not found: " + e.getHost());
                AppLogger.error(className, "  Check RPC_HOST configuration.");
            }
            case TIMEOUT -> {
                AppLogger.error(className, "✗ Bitcoind did not respond within "
                        + REQUEST_TIMEOUT_SECONDS + "s.");
                AppLogger.error(className, "  Method  : " + method);
                AppLogger.error(className, "  Action  : Check bitcoind is not overloaded.");
            }
            case SSL_ERROR -> {
                AppLogger.error(className, "✗ SSL/TLS error. Check node certificate.");
            }
            default -> {
                AppLogger.error(className, "✗ Unknown network error: " + e.getMessage());
            }
        }
        AppLogger.error(className, "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
    }
    ...
    private void waitBeforeRetry(int attempt) {
        if (attempt < MAX_RETRY_ATTEMPTS) {
            try {
                AppLogger.info(className, "Waiting " + RETRY_DELAY_MS + "ms before retry...");
